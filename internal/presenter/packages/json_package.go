...
type JSONPackage struct {
	jsonPackageBasicMetadata
	jsonPackageCustomMetadata
}

// jsonPackageBasicMetadata contains non-ambiguous values (type-wise) from pkg.Package.
type jsonPackageBasicMetadata struct {
	ID        string            `json:"id"`
...
// jsonPackageCustomMetadata contains ambiguous values (type-wise) from pkg.Package.
type jsonPackageCustomMetadata struct {
	MetadataType string      `json:"metadataType"`
	Metadata     interface{} `json:"metadata"`
}

// jsonPackageMetadataUnpacker is all values needed from JSONPackage to disambiguate ambiguous fields during json unmarshaling.
type jsonPackageMetadataUnpacker struct {
	MetadataType string          `json:"metadataType"`
...
func NewJSONPackage(p *pkg.Package) (JSONPackage, error) {
	...
	return JSONPackage{
		jsonPackageBasicMetadata: jsonPackageBasicMetadata{
			ID:        string(p.ID),
			Name:      p.Name,
			Version:   p.Version,
			Type:      string(p.Type),
		...
		jsonPackageCustomMetadata: jsonPackageCustomMetadata{
...
// ToPackage generates a pkg.Package from the current JSONPackage.
func (a JSONPackage) ToPackage() (pkg.Package, error) {
...
// nolint:funlen
func (a *JSONPackage) UnmarshalJSON(b []byte) error {
	var basic jsonPackageBasicMetadata
	if err := json.Unmarshal(b, &basic); err != nil {
		return err
	}
	a.jsonPackageBasicMetadata = basic

	var unpacker jsonPackageMetadataUnpacker
